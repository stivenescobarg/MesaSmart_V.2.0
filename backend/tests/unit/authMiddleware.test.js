...
// Unitario puro: no toca BD. Mockea jwt y el modelo Sesion.
...
describe("authMiddleware", () => {
  ...
  it("debería permitir un token válido sin jti", async () => {
    req.headers.authorization = "Bearer token123";
    ...
    expect(Sesion.estaActiva).not.toHaveBeenCalled();
  ...
  it("debería rechazar si la sesión no está activa", async () => {
    ...
    expect(res.status).toHaveBeenCalledWith(401);
    expect(res.json).toHaveBeenCalledWith({ msg: "Sesión expirada. Inicia sesión de nuevo." });
    expect(next).not.toHaveBeenCalled();
  });
